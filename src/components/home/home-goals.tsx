...
export function HomeGoals() {
  // Find the index of the first goal that's not done (up next)
  const upNextIndex = GOALS.findIndex(goal => !goal.done)
    ...
    <div className="w-full">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {GOALS.map((goal, index) => {
          const isUpNext = !goal.done && index === upNextIndex
          return (
            <div
              key={index}
              ...
              <div
                ...
                [{goal.done ? 'COMPLETE' : isUpNext ? 'UP NEXT' : 'PENDING'}]
              </div>
            </div>
          )
        })}
